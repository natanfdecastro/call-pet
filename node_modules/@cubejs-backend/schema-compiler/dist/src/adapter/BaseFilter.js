...
const momentRange = __importStar(require("moment-range"));
...
class BaseFilter extends BaseDimension_1.BaseDimension {
    ...
    filterParams() {
        if (ramda_1.contains(this.camelizeOperator, DATE_OPERATORS)) {
        ...
        if (this.isWildcardOperator()) {
            return ramda_1.map(this.escapeWildcardChars, params);
        }
        return params;
    }
    valuesArray() {
    ...
    likeOr(column, not) {
        return `${ramda_1.join(not ? ' AND ' : ' OR ', this.filterParams().map(p => this.likeIgnoreCase(column, not, p)))}${this.orIsNullCheck(column, not)}`;
    }
    ...
    inPlaceholders() {
        return `(${ramda_1.join(', ', this.filterParams().map(p => this.allocateCastParam(p)))})`;
    }
    inWhere(column) {
