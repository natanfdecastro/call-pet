...
class BaseQuery {
    ...
    externalPreAggregationQuery() {
        if (!this.options.preAggregationQuery && this.externalQueryClass) {
            ...
            return preAggregationsDescription.length && ramda_1.default.all((p) => p.external, preAggregationsDescription);
        }
        return false;
    }
    buildSqlAndParams() {
    ...
    fullKeyQueryAggregate() {
        ...
        let toJoin = (regularMeasures.length ? [
            ...
            this.regularMeasuresSubQuery.bind(this), measure)))(cumulativeMeasures));
        ...
        ([d, f]) => ({
            filterToWhere: () => {
                ...
                return f(isFromStartToEnd ?
                    ...
                    `${this.timeStampInClientTz(d.dateToParam())}`, `${d.convertedToTz()}`, `${this.timeStampInClientTz(d.dateFromParam())}`, `${this.timeStampInClientTz(d.dateToParam())}`, isFromStartToEnd);
            }
        }));
    ...
    overTimeSeriesSelect(cumulativeMeasures, dateSeriesSql, baseQuery, dateJoinConditionSql, baseQueryAlias) {
        const forSelect = this.dateSeriesSelect().concat(this.dimensions.concat(cumulativeMeasures).map(s => s.cumulativeSelectColumns())).filter(c => !!c).join(', ');
        ...
        return this.cacheValue(['preAggregationInvalidateKeyQueries', cube, JSON.stringify(preAggregation)], () => {
            ...
            if (preAggregation.refreshKey) {
                if (preAggregation.refreshKey.sql) {
                    return {
                        queries: [this.paramAllocator.buildSqlAndParams(preAggregationQueryForSql.evaluateSql(cube, preAggregation.refreshKey.sql))],
                ...
                if (preAggregation.refreshKey.incremental) {
                    ...
                    if (preAggregationQueryForSql.timeDimensions.length &&
                        ...
                        refreshKey = this.incrementalRefreshKey(preAggregationQueryForSql, refreshKey, { window: preAggregation.refreshKey.updateWindow });
                        renewalThreshold = this.incrementalRefreshKeyRenewalThreshold(preAggregationQueryForSql, renewalThreshold, preAggregation.refreshKey.updateWindow);
                    }
                }
            ...
            if (preAggregation.partitionGranularity &&
                ...
                const cubeFromPath = this.cubeEvaluator.cubeFromPath(cube);
                return preAggregationQueryForSql.evaluateSymbolSqlWithContext(() => preAggregationQueryForSql.cacheKeyQueries((originalRefreshKey, refreshKeyCube) => {
