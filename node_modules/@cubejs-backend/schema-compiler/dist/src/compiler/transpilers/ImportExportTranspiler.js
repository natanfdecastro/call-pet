            ...
            ExportNamedDeclaration(path) {
                ...
                const declarations = specifiers.map(specifier => {
                    if (specifier.node.type === 'ExportSpecifier') {
                        return t.objectProperty(specifier.get('exported').node, specifier.get('local').node);
                    }
                    else {
                        reporter.syntaxError({
                ...
                if (path.get('declaration')) {
                    path.replaceWithMultiple([
                        ...
                        t.callExpression(t.identifier('addExport'), [
                            t.objectExpression(path.get('declaration').get('declarations').map(d => t.objectProperty(d.get('id').node, d.get('id').node)))
                        ])
                    ]);
                }
                else {
