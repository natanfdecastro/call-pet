...
class RefreshScheduler {
    ...
    async refreshQueriesForPreAggregation(context, compilerApi, preAggregation, queryingOptions) {
        ...
        if (preAggregation.preAggregation.partitionGranularity) {
            ...
            const [startDate, endDate] = await Promise.all(compilerApi.createQueryByDataSource(compilers, queryingOptions, dataSource)
    ...
    async runScheduledRefresh(ctx, queryingOptions) {
        ...
        const context = {
            ...
            ...ctx,
            requestId: `scheduler-${ctx && ctx.requestId || v4_1.default()}`,
        };
    ...
    async roundRobinRefreshPreAggregationsQueryIterator(context, compilerApi, queryingOptions) {
        ...
        const queriesForPreAggregation = async (preAggregationIndex, timezone) => {
            ...
            if (!queriesCache[key]) {
                ...
                queriesCache[key] = this.refreshQueriesForPreAggregation(context, compilerApi, preAggregation, { ...queryingOptions, timezone });
            }
        ...
        return {
            ...
            current: async () => {
                ...
                if (partitionCursor < queries.length) {
                    ...
                    return {
                        ...sqlQuery,
                        preAggregations: sqlQuery.preAggregations.map((p) => ({ ...p, priority: preAggregationsWarmup ? 1 : queryCursor - queries.length })),
                        continueWait: true,
                        renewQuery: true,
                        requestId: context.requestId,
    ...
    async refreshPreAggregations(context, compilerApi, queryingOptions) {
        ...
        workerIndices = workerIndices || ramda_1.default.range(0, concurrency);
